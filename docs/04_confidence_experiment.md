...
## 실험 1: 첫 촬영 영상 (confidence_experiment.mp4`) - 조건 쉬움 
...
- **결론**: 좋은 촬영 조건(정면, 밝음, 단순 배경)에서는 confidence threshold가 결과에
  영향을 주지 않음 — 모델이 내는 실제 confidence 점수가 항상 threshold를 여유 있게
...
- **결론**: 손목 회전 각도가 크면 confidence threshold와 무관하게 인식이 실패함.
...
1. **confidence threshold는 기본값(0.5)을 그대로 사용**하는 것이 합리적. 조용한 환경에서는 threshold가 결과에 거의 영향이 없고, 빠른 움직임이 섞인 어려운 조건에서도 0.5가 감지율과 안정성 사이 균형이 가장 좋았음.
...
3. **모션 블러 대응**: 빠른 손 움직임으로 인한 순간적인 오판정은 confidence 튜닝으로
   ...
   인정하는 디바운스(debounce) 처리를 추가하면, 이런 찰나의 오판정이 자연스럽게 걸러질 것으로 예상됨 (아직 미구현).
...
- 테스트 영상: `confidence_experiment.mp4`(조건 쉬움) / `confidence_experiment2.mp4`(회전+흔들기) /
